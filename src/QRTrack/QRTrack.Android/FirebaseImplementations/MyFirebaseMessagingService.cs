    ...
    public class MyFirebaseMessagingService : FirebaseMessagingService
    {
        ...
        public override async void OnMessageReceived(RemoteMessage message)
        {
            ...
            //    bool isResponse = false;
                ...
                foreach (var item in message.Data)
                {
                    if (item.Key == "message")
                    {
                     //   isResponse = false;
                        ...
                        userId = strArr[0];
                        userIsAndroid = Convert.ToBoolean(strArr[1]);
                        text = strArr[2];
                        //if (item.Value.Contains("isResponse"))
                        //{
                        //    isResponse = true;
                        //    var strArr = item.Value.Split('/');
                        //    text = strArr[0];
                ...
                //    var tokenList = userDeviceTokenList.Where(s => s.UserId == userId).ToList().OrderByDescending(s => s.DateTime);
                ...
                //        if (userIsAndroid)
                //        {
                //            Services.PushNotiificationSenderService.SendAndroidPushNotification(token.Token, "Message to User Delivered / isResponse");

                //        }
                //        else
                //        {
                //            Services.PushNotiificationSenderService.SendIOSNotification(token.Token, "Message to User Delivered /isResponse");
